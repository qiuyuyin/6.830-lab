    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        for (int i = 1; i <= size; i++) {
            ...
            for (Set<LogicalJoinNode> set : sets) {
                ...
                for (LogicalJoinNode join : this.joins) {
                    if (set.contains(join)) {
                        CostCard costCard = this.computeCostAndCardOfSubplan(stats,
                                filterSelectivities, join, set, bestCostCard.cost, planCache);
                        if (costCard != null) {
        ...
        HashSet<LogicalJoinNode> res = new HashSet<>(joins);
        if (explain) {
            printJoins(planCache.getOrder(res), planCache, stats, filterSelectivities);
        }
        return planCache.getOrder(res);
    }
